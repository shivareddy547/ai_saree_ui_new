...
const CreateProduct: React.FC = () => {
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      const steps = [
        { progress: 10, msg: "Loading images..." },
        { progress: 25, msg: "Processing audio..." },
        { progress: 45, msg: "Building video timeline..." },
        { progress: 65, msg: "Rendering video..." },
        { progress: 85, msg: "Applying effects..." },
      ...
      // For demo, we create a fallback video blob (a short static black frame)
      ...
      ctx.fillText(productName || "Your Product", canvas.width / 2, canvas.height / 2 - 10);
      ctx.font = "16px sans-serif";
      ctx.fillText("₹" + (price || "0"), canvas.width / 2, canvas.height / 2 + 30);
      const blob = await new Promise<Blob | null>(resolve => canvas.toBlob(resolve, "video/mp4"));
      if (blob) {
        const url = URL.createObjectURL(blob);
        setVideoUrl(url);
        setVideoKitUrl(url);
